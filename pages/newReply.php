...
if(Input::exists()){
	if(Token::check(Input::get("token"))){
		...
		if($validate->passed()){
			...
			if(DB::getInstance()->insert('reply', [
				"unique_id" => $id,
				...
				Redirect::to("/post/".$post->data()->unique_id."/#{$id}");
			}else{
...
<body>
	...
	<div class="container-fluid mt-4">
		...
		<form action="" method="POST">
			<div class="form-group"><label for="title">Title:</label><input type="text" name="title" id="title" class="form-control" value="Re: <?php echo $post->data()->title;?>"></div>
